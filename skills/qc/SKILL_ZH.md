...
<!-- version: 0.3.1 | 同步规则：此文件的任何改动必须同步到 SKILL.md，反之亦然。 -->
...
1. 列出本次会话中所有被修改的文件
2. 对每个被修改的文件，搜索引用了它的其他文件——使用 Grep 搜索文件名/路径；检查 import/require/source 语句；搜索索引文件（MEMORY.md、CLAUDE.md、README.md、包清单）中的引用。范围：当前工作目录；若涉及配置文件则扩展到 `~/.claude/`。
...
**仅当**审查对象为完全独立的内容且无文件依赖时跳过此步（例如：独立的建议、尚未保存到文件的文档草稿、或不涉及现有文件的方案）。
...
**审查对象**：[自动识别/用户指定]
**额外标准**：[用户指定内容，无则省略此行]
**影响范围**：[N/A — 独立内容 | 扫描了 X 个文件；发现 Y 处过时引用]
