...
4. 每轮报告以状态头开始：`🔄 Round X/10 | Passes: Y/N | History: [P, M, m, P, ...]`（P=Pass, C=Critical, M=Major, m=Minor）
...
- **定稿前元校准**：在撰写总结部分之前，重新审视所有发现并自问：(1) 如果这条发现单独出现，我会给同样的严重性评级吗？(2) 我是否因发现太少而虚高评级，或因发现太多而压低评级？如有偏差，调整后再写总结。
